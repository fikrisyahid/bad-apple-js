  ...
  for (const [index, name] of fileNames.entries()) {
    ...
    for (let yFrame = 0; yFrame < 60; yFrame++) {
      for (let xFrame = 0; xFrame < 160; xFrame++) {
        ...
        for (let y = 0; y < 12; y++) {
          for (let x = 0; x < 6; x++) {
            charColor += (pixelData[yFrame * 12 + y][xFrame * 6 + x][0] + pixelData[yFrame * 12 + y][xFrame * 6 + x][1] + pixelData[yFrame * 12 + y][xFrame * 6 + x][2]) / (3 * 255);
          }
        }
        charColor /= (6 * 12);
        const char = illumination[Math.round(charColor * (illumination.length - 1))];
        frame += char;
      }
      frame += '\n';
    }
    frames.push(frame);
    console.clear();
    console.log(`${index} of ${fileNames.length} completed. ${(index / fileNames.length) * 100}%`);
  }
